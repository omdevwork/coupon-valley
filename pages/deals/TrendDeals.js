...
const TrendDeals = ({ data }) => {
  const [show, setShow] = useState(false);
  const clickHandler = () => {
    setShow(true);
  };

  const displayedData = show ? data : data?.slice(0, 4);
  const { setIsOpen, setSelectedCardData } = useSelectedCardContext();

  return (
    <div className="py-[24px] bg-[#e6edf2]">
      <div className="lg:max-w-[1140px] sm:max-w-[720px] max-w-[540px] mx-auto px-[14px]">
        ...
        <div className="grid lg:grid-cols-4 grid-cols-2 gap-x-[8px] gap-y-[12px] lg:gap-[18px] mb-[12px] lg:gap-y-[28px]">
          {displayedData?.map((item, key) => (
            ...
            >
              ...
              <p className="text-[#16171a] text-[12px] sm:text-[14px] h-[32px] overflow-hidden">
                {item.description}
              </p>
        ...
        <div className="text-center mt-[44px] mb-[24px]">
          <button
            className="border-[1px] hover:border-orange-500 hover:text-white transition-main hover:bg-orange-500 border-black bg-transparent max-w-[158px] w-full rounded-[3px] flex justify-center items-center mx-auto text-[14px] h-[42px]"
